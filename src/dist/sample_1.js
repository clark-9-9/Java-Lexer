...
function processFile(filePath) {
    try {
        ...
        try {
            ...
            console.error("Error during parsing, using fallback tokenizer:", parseError);
            lexerTokens = tokenizeJavaFallback(inputText);
        }
        // Convert tokens to required format
        const tokens = lexerTokens.map((token) => ({
 ...
 */
function tokenizeJavaFallback(sourceCode) {
    const tokens = [];
    // Java token patterns
    const patterns = [
        // Comments
 ...
 */
function categorizeToken(tokenType, tokenValue) {
    // Java keywords list
    const javaKeywords = [
        "abstract",
        "assert",
